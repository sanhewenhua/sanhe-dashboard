...
// 维度信息
const dimensions = [
  { key: 'revenueScore' as const, label: '收入力', max: 25, icon: <DollarOutlined />, color: '#faad14', desc: '月费收入规模' },
  { key: 'executionScore' as const, label: '执行力', max: 25, icon: <ThunderboltOutlined />, color: '#1677ff', desc: '上月完成率' },
  { key: 'roiScore' as const, label: '回报力', max: 25, icon: <FireOutlined />, color: '#f5222d', desc: '人均产值回报' },
  { key: 'paymentScore' as const, label: '回款力', max: 15, icon: <SafetyCertificateOutlined />, color: '#52c41a', desc: '回款健康度' },
  { key: 'stabilityScore' as const, label: '稳定力', max: 10, icon: <HistoryOutlined />, color: '#722ed1', desc: '连续合作稳定性' },
]
    ...
    <div>
      ...
      <Card size="small" style={{ marginBottom: 12, borderRadius: 10, background: '#fafafa' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: isMobile ? 8 : 16, fontSize: 12, color: '#595959' }}>
          ...
          <span style={{ color: '#8c8c8c' }}>
            · 总分100 = 收入力(25) + 执行力(25) + 回报力(25) + 回款力(15) + 稳定力(10)
          </span>
        </div>
      </Card>
      ...
      <Card size="small" style={{ marginTop: 16, borderRadius: 10, background: '#fffbe6', borderColor: '#ffe58f' }}>
        <div style={{ fontSize: 12, color: '#ad6800', lineHeight: 1.8 }}>
          ...
          · <strong>收入力</strong>：月费收入在全部项目中的相对规模<br />
          · <strong>执行力</strong>：上月实际完成率，完成/计划 × 25<br />
          · <strong>回报力</strong>：人均产值 = 上月已收金额 ÷ 参与人数，与全局人均对比<br />
          · <strong>回款力</strong>：上月回款比例，无应收默认中等<br />
          · <strong>稳定力</strong>：连续合作月数，暂停/有问题会扣分<br />
          · 已终止项目不参与排名，总分归零
        </div>
      </Card>
    </div>
